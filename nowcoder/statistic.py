...
class statistic_nowcoder : 
    ...
    def contest_submission_update(self, contest_id):
        ...
        return newest_submissions
    def get_contest_rank(self, contest_id, user) :
        ranking = []
    ...
    def contest_rank_update(self, contest_id) :
        ...
        def update_user_rank(user) :
            user_ranking = self.get_contest_rank(contest_id, user)
            if user_ranking :
                ranking.extend(user_ranking)
        ...
        with self.contest_rank_locker[f"{contest_id}"]:
            ...
            ranking.sort(key=lambda x: int(x["ranking"]))
    ...
    def add_contest(self, contest_id) :
        with self.contest_monitor_locker :
            ...
            for con in contest_info["contest"] :
                if str(contest_id) == con["contest_id"]:
                    return False
            contest_info["contest"].append({
    ...
    def delete_contest(self, contest_id) :
        with self.contest_monitor_locker :
            ...
            with open(file=contest_file, mode="r") as file:
                contest_info = json.load(file)
            tmp = [i for i in contest_info["contest"] if str(contest_id) != str(i["contest_id"])]
